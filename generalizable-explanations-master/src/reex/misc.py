...
def read_input_dataset(path):
    ...
    #    list_of_term_sets.append(set([row['word']]))
    return rd['Sentences']

def construct_fibo(path):

    g = Graph()
    g.parse(path)
    graph = rdflib_to_networkx_multidigraph(g)

    lem_en = Lemmatizer('en')
    map_nodes_to_their_lemmas = {}
    for node in graph.nodes:
        node_text = str(node).lower()
        tokens = node_text.split()
        node_lemmatized = ' '.join([lem_en.lemmatize(token) for token in tokens])
        map_nodes_to_their_lemmas[node] = node_lemmatized
        #print(node)

        #print(node_lemmatized)
        #print()

    graph = nx.relabel_nodes(graph, map_nodes_to_their_lemmas)
    #for node in graph.nodes:
    #    print(node)
    edges_to_remove = []
    edges_not_to_reverse = []
    for edge in graph.edges:
        if str(edge[0]) == "https://spec.edmcouncil.org/fibo/ontology/fbc/productsandservices/financialproductsandservices/trader" or str(edge[1]) == "https://spec.edmcouncil.org/fibo/ontology/fbc/productsandservices/financialproductsandservices/trader":
            print(edge)
        if str(edge[2]) == "http://www.w3.org/2000/01/rdf-schema#label":
            edges_not_to_reverse.append(edge)
        if str(edge[2]) != "http://www.w3.org/2000/01/rdf-schema#subClassOf" and str(edge[2]) != "https://spec.edmcouncil.org/fibo/ontology/FND/Relations/Relations/isProvidedBy" and str(edge[2]) != "http://www.w3.org/1999/02/22-rdf-syntax-ns#type" and str(edge[2]) != "https://www.omg.org/spec/LCC/Countries/CountryRepresentation/isUsedBy" and str(edge[2]) != "https://www.omg.org/spec/LCC/Languages/LanguageRepresentation/isMemberOf" and str(edge[2]) != "https://spec.edmcouncil.org/fibo/ontology/BE/GovernmentEntities/GovernmentEntities/hasJurisdiction" and str(edge[2]) != "https://www.omg.org/spec/LCC/Countries/CountryRepresentation/isPartOf":
            edges_to_remove.append(edge) 
            #print(edge)
    for edge in edges_to_remove:
        graph.remove_edge(edge[0], edge[1])
    #print(nx.algorithms.cycles.find_cycle(graph))

    print()
    print()
    ## REVERSE GRAPH
    reversed_graph = graph.reverse()
    for edge in edges_not_to_reverse:
        reversed_graph.add_edge(edge[0], edge[1], edge[2])
