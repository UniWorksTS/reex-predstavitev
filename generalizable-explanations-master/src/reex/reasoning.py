...
def selective_staircase_multiple_sets(list_of_termsets, ontology, intersectionRatio = 0, ontology_name = ''):
    ...
    while not all(v == 0 for v in converged):
        ...
        for enx in range(len(ancestor_storage)):
            ...
            if converged[enx] == 1:
                for term in termset:
                    try:
                        list_of_dicts[enx][term] = set([x[0] for x in ontology.in_edges(term)]) # or 1 and out edges
                    except Exception as e:
                        pass
                    #print(term, list_of_dicts[enx][term])
            ...
            if converged[ancestor_Set] == 1:
                ...
                for term in list_of_dicts[ancestor_Set].keys():
                    ...
                    for candidate in list_of_dicts[ancestor_Set][term]:
                        ...
                        if numberOfTerms == 0 or intersectionRatio >= float(intersectionCount) / float(numberOfTerms) and ('/' in str(candidate) or ontology_name != 'fibo'):
                            newSet.add(candidate)
                            term_used_in_generalization_step = True
                            #print(newSet)
                            #break # to only take 1
...
def extract_terms_for_input_generalization(explanations, ontology):
    term_list = []
    class_names = []
    for node in ontology.nodes:
        for sentence in explanations:
            if contains_word(sentence, node):
                term_list.append(set([node]))
                class_names.append(node)
                #print(node)
                #for edge in ontology.edges:
                #    if str(edge[0]) == str(node) or str(edge[1]) == str(node): 
                #        print(edge)
                #print("DONE")
                #print()
                break
    print(term_list)
    return term_list, class_names
...
def extract_terms_from_explanations(explanations, attributes, gene_to_go_map, min_terms, step, ontology, abs):
    ...
    for class_name, explanation_vector in explanations.items():
        ...
        threshold = maxVector
        print("Max vector value found: " + str(maxVector))
        while True:
            if not abs:
            ...
            if len(above_threshold) > min_terms or threshold < 0.03 * maxVector:
                ...
                print("Terms above threshold inside loop: " +str(above_threshold))
                break
            threshold *= step
        ...
        print("terms above threshold: " +str(terms))
        all_terms = set()
        if gene_to_go_map:
            for term in terms:
                try:
            ...
            all_terms = terms
        print("Selected terms for: " + str(class_name))
        print(all_terms)
        term_sets_per_class.append(all_terms)
        class_names.append(class_name)
        #print("per class: ", term_sets_per_class)

    return term_sets_per_class, class_names


def generalize_selective_staircase(ontology_graph, explanations = None, attributes = None, target_relations = {"is_a","partOf"}, test_run = False, intersectionRatio = 0, abs = False, print_results = False,gene_to_onto_map = None, min_terms = 5, step = 0.9, generalizing_input=False, ontology_name=''):
    ...
    if generalizing_input:
        term_sets_per_class, class_names = extract_terms_for_input_generalization(explanations, ontology_graph)
    else:
    ...
    subsets = selective_staircase_multiple_sets(term_sets_per_class, ontology_graph, intersectionRatio = intersectionRatio, ontology_name=ontology_name)
        ...
        for enx, termset in enumerate(ancestor_storage):
            ...
            if converged[enx] == 1:
                ...
                if len(pairAncestorSet) > 0:
                    print(ancestor_storage[enx])
                    ancestor_storage[enx] = pairAncestorSet
                    print(pairAncestorSet)
                    print(len(pairAncestorSet))
